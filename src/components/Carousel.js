    ...
    <div>
      ...
      <div className="row m-2 p-1">
        <div className={"p-1 col" + (activeImage === 0 ? " text-bg-primary" : "")}>
          <img onClick={() => setActiveImage(0)} src={(isProd ? "/TogetherAgainstCancer" : "") + "/graphs/" + (currentCancer.type).toLowerCase() + "/daly.png"} className="mx-auto d-block w-100" />
        </div>
        <div className={"p-1 col" + (activeImage === 1 ? " text-bg-primary" : "")}>
          <img onClick={() => setActiveImage(1)} src={(isProd ? "/TogetherAgainstCancer" : "") + "/graphs/" + (currentCancer.type).toLowerCase() + "/hbar.png"} className="mx-auto d-block w-100" />
        </div>
